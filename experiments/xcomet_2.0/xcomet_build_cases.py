...
def _diff_spans(good: str, bad: str) -> List[Dict[str, Any]]:
    """Return character-level diff spans from good->bad (exclusive end)."""
    matcher = SequenceMatcher(None, good or "", bad or "", autojunk=False)
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        ...
        spans.append({
            ...
            "end": j2,
            "tag": tag,
            "bad_text": (bad or "")[j1:j2],
            "good_text": (good or "")[i1:i2],
        })
    return spans


def _attach_gt_spans(cases: Dict[str, Dict[str, Any]]) -> None:
    """
    For each case, if no manual GT spans are provided, auto-fill:
      - gt_error_spans_full_good: []
      - gt_error_spans_full_bad: diff(good_full, bad_full)
      - gt_error_spans_bad_by_seg: per-segment diff(good_seg, bad_seg)
    """
    for case in cases.values():
        mt_full_good = case.get("mt_full_good", "")
        mt_full_bad = case.get("mt_full_bad", "")

        # Only fill when user didn't provide manual annotations.
        if "gt_error_spans_full_good" not in case:
            case["gt_error_spans_full_good"] = []
        if "gt_error_spans_full_bad" not in case:
            case["gt_error_spans_full_bad"] = _diff_spans(mt_full_good, mt_full_bad)

        # Per-segment GT (used when caller prefers segment-level offsets)
        if "gt_error_spans_bad_by_seg" not in case:
            mt_segs_good = case.get("mt_segs_good", [])
            mt_segs_bad = case.get("mt_segs_bad", [])
            seg_spans: List[List[Dict[str, Any]]] = []
            for g, b in zip(mt_segs_good, mt_segs_bad):
                seg_spans.append(_diff_spans(g, b))
            case["gt_error_spans_bad_by_seg"] = seg_spans
        if "gt_error_spans_good_by_seg" not in case:
            mt_segs_good = case.get("mt_segs_good", [])
            case["gt_error_spans_good_by_seg"] = [[] for _ in mt_segs_good]
...
def build_wmt24_100_cases() -> Dict[str, Dict[str, Any]]:
    ...
    # Auto-fill GT spans when manual annotations are absent.
    _attach_gt_spans(cases)
