...
class QwenGenerator:
    """Qwen模型生成器，支持draft mode和repair mode"""
        ...
        if self.use_vllm and VLLM_AVAILABLE:
            ...
            if not torch.cuda.is_available():
                ...
                try:
                    ...
                    if self.device.startswith("cuda") and torch.cuda.is_available():
                        try:
                            ...
                            for visible_gpu_id in range(num_visible_gpus):
                                # 获取对应的物理 GPU ID
                                physical_gpu_id = (
                                    original_physical_gpu_ids[visible_gpu_id]
                                    if visible_gpu_id < len(original_physical_gpu_ids)
                                    else visible_gpu_id
                                )
                            ...
                            for info in gpu_memory_info:
                                ...
                                print(
                                    f"[Qwen]   可见 GPU {info['visible_gpu_id']} (物理 GPU {info['physical_gpu_id']}): "
                            ...
                            if sufficient_gpus:
                                ...
                                print(
                                    f"[Qwen] 自动选择物理 GPU {selected_physical_gpu_id}"
                                    f"（可见 GPU {selected_gpu_info['visible_gpu_id']}，"
                                ...
                                actual_util = max(
                                    0.3, (selected_gpu_info["free"] - 1.0) / selected_gpu_info["total"]
                                )
                                kwargs["gpu_memory_utilization"] = actual_util
                                print(
                                    f"[Qwen] 所有 GPU 内存都不足，选择物理 GPU {selected_physical_gpu_id}"
                                    f"（可见 GPU {selected_gpu_info['visible_gpu_id']}，"
                                    f"可用内存最多: {selected_gpu_info['free']:.2f} GB）"
                                )
                                print(
                                    f"[Qwen] 自动降低 gpu_memory_utilization: "
                                    f"{requested_gpu_memory_util} -> {actual_util:.2f}"
                                )
                            ...
                            if selected_physical_gpu_id is not None:
                                ...
                                print(
                                    f"[Qwen] 更新 CUDA_VISIBLE_DEVICES={selected_physical_gpu_id}"
                                    f"（vLLM 将使用此 GPU）"
                                )
                    ...
                    if "dtype" not in kwargs:
                        ...
                        try:
                            ...
                            if preferred_dtype == "float16":
                                print(
                                    f"[Qwen] 检测到设备 {device_name} (cc {major}.{minor})，"
                                    f"自动使用float16以避免bfloat16导致的数值问题"
                                )
                            else:
                    ...
                    # max_model_len 是「这个 vLLM 引擎一次能容纳的最大序列长度（prompt + 输出）」的上限，
                    # 用来在初始化时决定 KV cache 的大小和显存占用。是“能力上限、硬上限”。
                    default_max_model_len = kwargs.get("max_model_len", 16384)
                    if default_max_model_len > 32768:
                        print(
                            f"[Qwen] Warning: max_model_len={default_max_model_len} 可能过大，"
                            f"将限制到 16384"
                        )
                    ...
                    vllm_kwargs = {
                        ...
                        "enforce_eager": kwargs.get("enforce_eager", True),  # test_time 中设置
                    }
                    # 移除None值
                    vllm_kwargs = {k: v for k, v in vllm_kwargs.items() if v is not None}

                    print(
                        "[Qwen] Loading vLLM with "
                        f"tensor_parallel_size={vllm_kwargs.get('tensor_parallel_size')}, "
                        f"gpu_memory_utilization={vllm_kwargs.get('gpu_memory_utilization')}"
                    )
                    ...
                    if "No available memory for the cache blocks" in error_msg or "KV cache" in error_msg:
                        print("[Qwen] KV cache 内存不足。建议：")
                        print("[Qwen]   1. 减少 GPU 数量（如使用 2 个 GPU：--qwen_gpus 0,1）")
                        print(
                            "[Qwen]   2. 降低 gpu_memory_utilization（当前: "
                            f"{kwargs.get('gpu_memory_utilization', self.gpu_memory_utilization)}，建议: 0.7）"
                        )
                        print("[Qwen]   3. 或者使用 transformers 后端（较慢但更稳定）")
                    elif "Free memory on device" in error_msg:
                        print("[Qwen] GPU 内存不足。建议：")
                        print("[Qwen]   1. 清理 GPU 上的其他进程（如 XCOMET）")
                        print(
                            "[Qwen]   2. 降低 gpu_memory_utilization（当前: "
                            f"{kwargs.get('gpu_memory_utilization', self.gpu_memory_utilization)}，建议: 0.7）"
                        )
                        print("[Qwen]   3. 使用空闲的 GPU（如 --qwen_gpus 4）")
                        print("[Qwen]   4. 或者使用 transformers 后端（较慢但更稳定）")
                    elif "ValueError" in error_msg and (
                        "invalid literal" in error_msg or "CUDA_VISIBLE_DEVICES" in error_msg
                    ):
                        print("[Qwen] 这可能是CUDA_VISIBLE_DEVICES格式问题，请检查环境变量格式")
                        print(
                            f"[Qwen] 当前CUDA_VISIBLE_DEVICES: "
                            f"{os.environ.get('CUDA_VISIBLE_DEVICES', '未设置')}"
                        )
                    print("[Qwen] Falling back to transformers backend")
                    self.use_vllm = False
    ...
    def generate_draft(
        ...
        mode: str = "draft",      # mode: "draft" / "repair"
        max_tokens: int = 2048,
        ...
        **kwargs
        ...
        Args:
            prompts: 单个prompt字符串或prompt列表（已经是指令模板，如rl/repair模板）
            mode: "draft" 表示初稿翻译（不启用thinking）；
                  "repair" 表示润色模式（启用thinking）
        ...
        should_sample = (
            explicit_do_sample if explicit_do_sample is not None else (raw_temperature > 0.0 and raw_top_p < 1.0)
        )
        ...
        # === 3. 根据mode决定是否启用thinking ===
        # Qwen3 官方接口：enable_thinking=True / False
        enable_thinking = (mode == "repair")

        if self.use_vllm and self.vllm_engine:
            # ------------------- vLLM 路径：先包一层 chat_template -------------------
            chat_prompts = []
            for p in prompts:
                ...
                text = self.tokenizer.apply_chat_template(
                    ...
                    enable_thinking=enable_thinking,  # draft: False, repair: True
                )
            ...
            outputs = self.vllm_engine.generate(chat_prompts, sampling_params)
            ...
            # 清理采样参数，避免与transformers重复
            sampling_param_keys = {"temperature", "top_p", "top_k", "do_sample"}
            cleaned_kwargs = {k: v for k, v in kwargs.items() if k not in sampling_param_keys}

            for idx, prompt in enumerate(prompts):
                # print(f"[生成进度] {idx + 1}/{len(prompts)}") 

                # 3.1 构建chat格式 + Qwen3 thinking 开关
                messages = [{"role": "user", "content": prompt}]
                text = self.tokenizer.apply_chat_template(
                    messages,
                    tokenize=False,
                    add_generation_prompt=True,
                    enable_thinking=enable_thinking,  # 核心修改：用mode控制thinking
                )
                ...
                with torch.no_grad():
                    ...
                    try:
                        ...
                        if any(
                            ...
                            print("[Warning] 生成时遇到数值/CUDA问题，切换到greedy decoding重试...")
    ...
    def generate_from_source(
        ...
        **generation_kwargs,
        ...
        Args:
            ...
            mode: 生成模式（默认"draft"，也可以传"repair"，外部会用repair模板）
        ...
        for src, lg in zip(source_texts, lang_pair):
            ...
            # 使用rl模板构建prompt（draft mode复用rl模板；repair时你外部会传repair模板）
            example = {
                "lg": lg,
                "src_text": src,
            }
